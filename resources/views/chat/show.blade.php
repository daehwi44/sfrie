<x-app-layout>
    ...
    <div class="container mx-auto px-4">
        <div class="py-8">
            <div class="overflow-y-auto" style="max-height: 40rem;" id="message-list">
                @foreach ($messages as $message)
                    <div class="flex items-start mb-4 {{ $message->from_user_id == Auth::id() ? 'justify-end' : '' }}">
                        <div
                            class="ml-20 mr-8 w-80  {{ $message->from_user_id == Auth::id() ? 'bg-green-500 text-white' : 'bg-gray-400 text-gray-800' }} rounded-lg p-3 shadow-md">
                            <p class="text-sm">{{ $message->message }}</p>
                            <span class="text-xs text-gray-900">{{ $message->created_at->format('Y/m/d H:i') }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="py-4">
                <form method="POST" action="{{ route('chat.send', $user->id) }}">
                    ...
                    <div class="flex items-center rounded-md">
                        ...
                        <button
                            class="flex-shrink-0 bg-teal-500 hover:bg-teal-700 text-sm border-4 border-teal-500 hover:border-teal-700 text-white py-1 px-2 rounded shadow-md"
                            type="submit">
                            送信
                        </button>
                    </div>
